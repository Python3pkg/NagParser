...
class NagiosDefinition(object):
    DATETIME_FORMAT = '%Y-%m-%d %H:%M:%S'
    STALETHRESHOLD = 240     #Threshold in seconds we consider data current
    IGNORESTALEDATA = False

    #these need to move to a config file
    ...
    def getbadservices(self):
        ...
        #return self.getbad(Nagios.Service, self.services)
    pass
    ...
    def genoutput(self, outputformat = 'xml', returnxmldocument = True, recursive = True, items = None):
        ...
        for attr in self.attributes:
            if not isinstance(attr[1], Nagios) and not attr[1] == None:
                if outputformat == 'xml':
                    output.setAttribute(attr[0], attr[1])

        order = ['host', 'service']

        if recursive and (selfclassname != order[1] ):
            if items == None:
                if order[0] == selfclassname:
                    items = getattr(self, order[1]+'s')
                else:
                    if selfclassname == 'servicegroup':
                        items = getattr(self, order[1]+'s')
                    else:
                        items = getattr(self, order[0]+'s')

            for obj in items:
                temp = obj.genoutput(outputformat = outputformat, returnxmldocument = False)
                if outputformat == 'xml':
    ...
    def getservice(self, service_description):
        return self.getobj(objtype = Nagios.Service, value = service_description, attribute = 'service_description', first = True)
    ...
    def scheduledowntime(self, author, starttime, endtime, comment, apikey = None, doappend = False):
        ...
        if self.classname() == 'servicegroup':
            ...
            return 'Error: Invalid Nagios object'
        ...
        if doappend:
            try:
                if self.nagios.APIKEY == None or apikey not in self.nagios.APIKEY:
...
class Nagios(NagiosDefinition):
    ...
    def gethost(self, host_name):
        return self.getobj(objtype = Nagios.Host, value = host_name, attribute = 'host_name', first = True)

    def getservicegroup(self, servicegroup_name):
        return self.getobj(objtype = Nagios.ServiceGroup, value = servicegroup_name, attribute = 'servicegroup_name', first = True)

    def getbadhosts(self):
        return self.getbad(Nagios.Host)
    ...
    class Host(NagiosDefinition):
        def _get_services(self):
            return filter(lambda x: x.host_name == self.host_name, self.nagios.services)
    ...
    class Service(NagiosDefinition):
        def _get_host(self):
            host = filter(lambda x: x.host_name == self.host_name, self.nagios.hosts)
            if len(host):
                host = host[0]
            return host
    ...
    class ServiceGroup(NagiosDefinition):
        def _get_services(self):
            tempservices = []
            ...
            for i in range(len(members)):
                if i % 2 == 0:
                    tempservices.append(self.nagios.gethost(members[i]).getservice(members[i+1]))
